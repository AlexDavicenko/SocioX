#include <pybind11/pybind11.h>

#include <Windows.h>

#include <iostream>
#include <fstream>
#include <string>
#include <random>
#include <cstring>

using namespace std;

char* format_pass(char* password) {

    char* result = new char[73];
    memset(result, '-', 73);
    result[72] = '\0';

    for (int i = 0; password[i] != '\0'; i++) {
        result[i] = password[i];
    }


    return result;

}

uint32_t convert_char_array_to_int32(char* array, int start, int size) {
    start = start % size;

    uint32_t out = 0;

    for (int i = 0; i < 4; i++) {
        out = out | (static_cast<unsigned char>(array[(start + i) % size]) << (i * 8));
    }

    return out;
}

uint64_t convert_char_array_to_int64(char* array, int start) {
    uint64_t out = 0;

    for (int i = 0; i < 8; i++) {
        uint64_t c = static_cast<unsigned char>(array[i+ start]);
        out = out | (c << (i * 8));
    }

    return out;
}


uint32_t F(uint32_t x, uint32_t Sboxes[4][256]) {

    uint8_t a;
    uint8_t b;
    uint8_t c;
    uint8_t d;
    uint32_t y;

    d = x & 0x00FF;
    x = x >> 8;
    c = x & 0x00FF;
    x = x >> 8;
    b = x & 0x00FF;
    x = x >> 8;
    a = x & 0x00FF;

    y = Sboxes[0][a] + Sboxes[1][b];
    y = y ^ Sboxes[2][c];
    y = y + Sboxes[3][d];
    return y;
}

uint64_t Encrypt(uint32_t Pboxes[18], uint32_t Sboxes[4][256], uint64_t block) {

    uint32_t R = block;
    uint32_t L = block >> 32;

    for (int i = 0; i < 16; i++) {

        L = L ^ Pboxes[i];

    
        //F function
        uint32_t f = F(L, Sboxes);
        R = f ^ R;

        //Swap L and R
        uint32_t tmp = R;
        R = L;
        L = tmp;

    }

    R = R ^ Pboxes[16];
    L = L ^ Pboxes[17];


    uint64_t out = L;
    out = out << 32;
    out = out | R;

    return out;

}


void ExpandKey(uint32_t Pboxes[18], uint32_t Sboxes[4][256], char* password, int size, char* salt) {

    for (int i = 0; i < 18; i++) {

        uint32_t password_chunk = convert_char_array_to_int32(password, 4 * i, size);
        Pboxes[i] = Pboxes[i] ^ password_chunk;

    }

    uint64_t block = 0;
    uint64_t salthalf1 = convert_char_array_to_int64(salt, 0);
    uint64_t salthalf2 = convert_char_array_to_int64(salt, 8);

    for (int i = 0; i < 9; i++) {

        if (i % 2 == 0) {
            block = block ^ salthalf2;
        }
        else {
            block = block ^ salthalf1;
        }

        block = Encrypt(Pboxes, Sboxes, block);


        Pboxes[2 * i] = block;
        Pboxes[2 * i + 1] = block >> 32;
    }

    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 128; j++) {

            if (j % 2 == 0) {
                block = block ^ salthalf1;
            }
            else {
                block = block ^ salthalf2;
            }

            block = Encrypt(Pboxes, Sboxes, block);

            Sboxes[i][2 * j] = block;
            Sboxes[i][2 * j + 1] = block >> 32;

        }
    }
}


void ExpensiveBlowfishSetup(int cost, uint32_t Pboxes[18], uint32_t Sboxes[4][256], char* password, char* salt) {

    string seed = "";

    int offset = 0;
    char buffer[9];
    char* end;
    buffer[8] = '\0';

    for (int i = 0; i < 18; i++) {

        for (int j = 0; j < 8; j++) {
            buffer[j] = seed[offset + j];
        }
        //Convert buffer into string
        uint32_t intBuffer = strtoul(buffer, &end, 16);
        Pboxes[i] = intBuffer;
        offset += 8;
    }

    for (int i = 0; i < 256 * 4; i++) {

        for (int j = 0; j < 8; j++) {
            buffer[j] = seed[offset + j];
        }


        uint32_t intValue = strtoul(buffer, &end, 16);
        Sboxes[i / 256][i % 256] = intValue;
        offset += 8;
    }

    ExpandKey(Pboxes, Sboxes, password, 72, salt);

    char* emtpySalt = new char[16];
    memset(emtpySalt, '\0', 16);

    for (int i = 0; i < (1 << cost); i++) {
        ExpandKey(Pboxes, Sboxes, password, 72, emtpySalt);
        ExpandKey(Pboxes, Sboxes, salt, 16, emtpySalt);

    }

}

char* int32_to_hex(int value) {
    char* hex_string = new char[9];
    memset(hex_string, '0', 9);
    hex_string[8] = '\0';
    sprintf_s(hex_string, 9, "%x", value);
    return hex_string;

}
char* int64_to_hex(uint64_t value) {
    char* hex_string = new char[17];
    memset(hex_string, '0', 17);
    hex_string[16] = '\0';
    sprintf_s(hex_string, 17, "%llx", value);
    return hex_string;

}

uint32_t hex_to_int32(char* hex_string) {
    char* end;
    return strtoul(hex_string, &end, 16);
    
}

uint64_t hex_to_int64(char* hex_string) {
    char* end;
    return strtoul(hex_string, &end, 16);
}

char* convert_hex_salt_to_ascii_salt(char* hex_salt) {
    //32 Hex digits
    char* buffer = new char[9];
    char* salt = new char[17];
    memset(buffer, '0', 9);
    memset(salt, '0', 17);
    salt[16] = '\0';
    buffer[8] = '\0';

    for (int i = 0; i < 16; i++) {

        //Set to least signficiant bits
        buffer[7] = hex_salt[2*i];
        buffer[6] = hex_salt[2*i+1];

        salt[i] = static_cast<char>(hex_to_int32(buffer));
    }

    return salt;
}

char* bcrypt(char* password, char* hex_salt, int cost) {

    //Change password and salt into correct format
    password = format_pass(password);
    

    char* salt = convert_hex_salt_to_ascii_salt(hex_salt);

    //Define S and P boxes
    uint32_t Pboxes[18];
    uint32_t SBoxes[4][256];

    memset(Pboxes, 0, sizeof(Pboxes));
    memset(SBoxes, 0, sizeof(SBoxes));


    ExpensiveBlowfishSetup(cost, Pboxes, SBoxes, password, salt);

    //Encrypt arbitrary string with blocks
    char ctext[] = "OrpheanBeholderScryDoubt";

    //Convert into ints
    uint64_t block0 = convert_char_array_to_int64(ctext, 0);
    uint64_t block1 = convert_char_array_to_int64(ctext, 8);
    uint64_t block2 = convert_char_array_to_int64(ctext, 16);


    for (int i = 0; i < 64; i++) {
        block0 = Encrypt(Pboxes, SBoxes, block0);
        block1 = Encrypt(Pboxes, SBoxes, block1);
        block2 = Encrypt(Pboxes, SBoxes, block2);
    }


    char* hash = new char[25];
    hash[24] = '\0';

    char* hex0 = int64_to_hex(block0);
    char* hex1 = int64_to_hex(block1);
    char* hex2 = int64_to_hex(block2);

    for (int i = 0; i < 8; i++) {
        hash[i] = hex0[i];
        hash[i + 8] = hex1[i];
        hash[i + 16] = hex2[i];
    }

    return hash;


}


char* generate_random_salt() {

    random_device rd;
    mt19937 generator(rd());

    char* salt = new char[33];
    memset(salt, '0', 32);
    salt[32] = '\0';

    for (int i = 0; i < 4; i++) {

        char* hex_digits = int32_to_hex(generator());

        int k = 0;
        while (k < 8) {

            //IDK HOW THIS WORKS BUT IT WORKS
            if (hex_digits[k] == '\0' || static_cast<int>(hex_digits[k]) == -2) {
                *(hex_digits + k) = '0';
            }
            k++;
        }
        hex_digits[8] = '\0';

        for (int j = 0; j < 8; j++) {
            salt[8 * i + j] = hex_digits[j];
        }
    }

    return salt;
}

//Exporting functions
PYBIND11_MODULE(bcryptCPP, m) {
    m.def("bcrypt", &bcrypt,
        "Compute the bcrypt hash of 64 character long password\n Password: str\n Salt: str(Hex)\nCost: int");

    m.def("generate_random_salt", &generate_random_salt,
        "Generate a random salt value of 16 bytes, represented by a 32-digit hexadecimal string");


#ifdef VERSION_INFO
    m.attr("__version__") = VERSION_INFO;
#else
    m.attr("__version__") = "dev";
#endif
}