...
class TextBarFrame(ctk.CTkFrame):
    ...
    def window_bindings(self) -> None:
        ...
        self.controller.add_binding(
            '<F1>', lambda e : self.on_suggestion_press(e=None, suggestionNo=1))
        self.controller.add_binding(
            '<F2>', lambda e : self.on_suggestion_press(e=None, suggestionNo=2))
        self.controller.add_binding(
            '<F3>', lambda e : self.on_suggestion_press(e=None, suggestionNo=3))
    ...
    def on_message_entry_box_modify(self, e = None):
        words = self.controller.get_suggestions(
            self.message_box_text_variable.get().split(' ')[-1])
...
class SuggestionFrame(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkFrame, controller: Controller):
        ...
        self.suggestion_box_1 = ctk.CTkButton(
            self, text = '', fg_color='#454447', 
            command= lambda : controller.on_suggestion_press(suggestionNo=1))
        self.suggestion_box_2 = ctk.CTkButton(
            self, text = '', fg_color='#454447', 
            command= lambda : controller.on_suggestion_press(suggestionNo=2))
        self.suggestion_box_3 = ctk.CTkButton(
            self, text = '', fg_color='#454447', 
            command= lambda : controller.on_suggestion_press(suggestionNo=3))
